...
export class DiscordAlerter {
	constructor({ botToken, channelId, channelIds, guildId, guildIds, webhookUrl, webhookUrls, applicationId }) {
		this.botToken = botToken;
		this.channelIds = normalizeTargets(channelIds, channelId);
		this.guildIds = normalizeTargets(guildIds, guildId);
		this.webhookUrls = normalizeTargets(webhookUrls, webhookUrl);
		this.applicationId = applicationId || null;
		this.channelId = this.channelIds[0] || null;
		this.guildId = this.guildIds[0] || null;
	...
	get isConfigured() {
		return !!(this.botToken && this.channelIds.length > 0) || this.webhookUrls.length > 0;
	}
	...
	async start() {
		...
		try {
			...
			// Wait for READY before command registration so app/user IDs are available.
			...
			// Register slash commands after ready to avoid invalid application_id fallback.
	...
	async _registerCommands(REST, Routes, SlashCommandBuilder) {
		...
		const appId = this.applicationId || this._client?.application?.id || this._client?.user?.id;

		if (!appId) {
			console.warn('[Discord] Skipping slash command registration: missing application ID');
			return;
		}
		...
		try {
			if (this.guildIds.length > 0) {
				...
				console.log(`[Discord] Registered ${commands.length} guild slash commands in ${this.guildIds.length} guild(s)`);
			} else {
	...
	async _handleCommand(interaction) {
		...
		if (name === 'alerts') {
			...
			if (recent.length === 0) {
				await interaction.reply({ content: 'No recent alerts.', flags: 64 });
				return;
			}
		...
		if (handler) {
			...
			await interaction.reply({ content: `Unknown command: /${name}`, flags: 64 });
		}
	}
	 ...
	 * Works with the bot client or falls back to webhook URLs.
	 */
	async sendMessage(content, embeds = []) {
		...
		if (this._ready && this._client && this.channelIds.length > 0) {
			for (const channelId of this.channelIds) {
				try {
					...
					console.error(`[Discord] Send via bot failed for channel ${channelId}:`, err.message);
				}
			}
		...
		if (this.webhookUrls.length > 0) {
			for (const url of this.webhookUrls) {
				const ok = await this._sendWebhook(url, content, embeds);
				if (ok) sentAny = true;
			}
			return sentAny;
		}

		console.warn('[Discord] Cannot send — bot not ready and no webhook URLs configured');
		return false;
	}
	...
	_ruleBasedEvaluation(signals, delta) {
		...
		if (nukeSignal) {
			return {
				shouldAlert: true, tier: 'FLASH', confidence: 'HIGH', headline: 'Nuclear Anomaly Detected',
...
function normalizeTargets(multiValue, singleValue) {
	const values = [];
	...
	const pushTokens = (value) => {
		...
		if (Array.isArray(value)) {
			for (const token of value) pushTokens(token);
			return;
		}
	...
	pushTokens(multiValue);
	pushTokens(singleValue);
