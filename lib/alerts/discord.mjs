...
export class DiscordAlerter {
  constructor({ botToken, channelId, channelIds, guildId, guildIds, webhookUrl, webhookUrls }) {
    ...
    try {
      if (this.guildIds.length > 0) {
        ...
        for (const guildId of this.guildIds) {
          await rest.put(Routes.applicationGuildCommands(this._client?.user?.id || 'me', guildId), { body: commands });
        }
