...
export async function synthesize(data) {
	...
	// German Data Sources - DWD Weather Alerts
	const dwdData = data.sources.DWD || {};
	const dwd = {
		totalAlerts: (dwdData.alerts || []).length,
		severityCounts: {
			extreme: (dwdData.alerts || []).filter(a => a.severity === 'Extreme').length,
			severe: (dwdData.alerts || []).filter(a => a.severity === 'Severe').length,
			moderate: (dwdData.alerts || []).filter(a => a.severity === 'Moderate').length,
			minor: (dwdData.alerts || []).filter(a => a.severity === 'Minor').length,
		},
		topAlerts: (dwdData.alerts || []).slice(0, 5).map(a => ({
			event: a.event, severity: a.severity, headline: a.headline?.substring(0, 80)
		})),
		signals: dwdData.signals || []
	};

	// German Power Grid - SMARD
	const smardData = data.sources.SMARD || {};
	const smard = {
		renewable: smardData.powerGeneration?.renewable || 0,
		fossil: smardData.powerGeneration?.fossil || 0,
		nuclear: smardData.powerGeneration?.nuclear || 0,
		spotPrice: smardData.spotPrice?.current || 0,
		spotPriceTrend: smardData.spotPrice?.trend || 'stable',
		gridLoad: smardData.gridLoad?.percentUsed || 0,
		signals: smardData.signals || []
	};

	// German Economic Indicators - Destatis
	const destatisfData = data.sources.Destatis || {};
	const destatis = {
		cpi: destatisfData.indicators?.cpi?.yearOverYear || 0,
		unemployment: destatisfData.indicators?.unemployment?.rate || 0,
		industrialProduction: destatisfData.indicators?.industrialProduction?.yearOverYear || 0,
		population: destatisfData.indicators?.population?.total || 0,
		signals: destatisfData.signals || []
	};

	// EU Economic Context - Eurostat
	const eurostatData = data.sources.Eurostat || {};
	const eurostat = {
		germanyVsEU: {
			cpi: {
				germany: eurostatData.indicators?.cpi?.germany || 0,
				euAverage: eurostatData.indicators?.cpi?.eu27Average || 0,
			},
			unemployment: {
				germany: eurostatData.indicators?.unemployment?.germany || 0,
				euAverage: eurostatData.indicators?.unemployment?.eu27Average || 0,
			},
			industrialProd: {
				germany: eurostatData.indicators?.industrialProd?.germany || 0,
				euAverage: eurostatData.indicators?.industrialProd?.eu27Average || 0,
			}
		},
		signals: eurostatData.signals || []
	};

	// Location Service - Nominatim Geocoding
	const nominatimData = data.sources.Nominatim || {};
	const nominatim = {
		apiHealthy: nominatimData.status?.apiHealthy || false,
		cacheEntries: nominatimData.cache?.entries || 0,
		cacheMaxSize: nominatimData.cache?.maxSize || 1000,
		cacheUtilization: ((nominatimData.cache?.entries || 0) / (nominatimData.cache?.maxSize || 1) * 100).toFixed(0),
		signals: nominatimData.signals || []
	};
	...
	const V2 = {
		...
		// German Data Sources
		dwd, smard, destatis, eurostat, nominatim,
